...
export class FireProvider extends ObservableV2<any> {
  ...
  private hydrateRetryAttempt: number = 0;
  ...
  private scheduleHydrateRetry = () => {
    const delay = Math.min(
      SNAPSHOT_BACKOFF_BASE_MS * Math.pow(2, this.hydrateRetryAttempt),
      SNAPSHOT_BACKOFF_MAX_MS,
    );
    this.hydrateRetryAttempt++;
    this.consoleHandler(
      "Scheduling snapshot hydrate retry",
      `attempt ${this.hydrateRetryAttempt}, delay ${delay}ms`,
    );
    if (this.snapshotRetryTimeout) clearTimeout(this.snapshotRetryTimeout);
    this.snapshotRetryTimeout = setTimeout(() => {
      this.trackData();
  ...
  private handleDocSnapshot = async (snap: {
    exists: () => boolean;
    data: () => unknown;
   ...
   */
  private applyRemoteSnapshot = async (
    meta: ReturnType<typeof readSnapshotMeta>,
    hydrateGen: number,
  ): Promise<boolean> => {
    if (this.snapshotStore) {
      let stored = snapshotMetaFromFields(meta);
      let defaultBytes: Uint8Array | undefined;
      if (!stored && this.snapshotStore.readDefault) {
        try {
          const fallback = await this.snapshotStore.readDefault({
            epoch: meta.epoch,
          });
          if (
            fallback &&
        ...
        if (hydrateGen !== this.snapshotHydrateGen) return false;
      }
      if (!stored) {
      ...
      if (!bytes) {
        try {
          bytes = await this.snapshotStore.read(stored);
        } catch (error) {
          this.consoleHandler("Firestore sync error", error);
          this.scheduleHydrateRetry();
          return false;
   ...
   */
  adoptPersistedSnapshot = (bytes: Uint8Array) => {
    if (bytes.byteLength <= EMPTY_YJS_UPDATE_MAX_BYTES) return;
    Y.applyUpdate(this.doc, bytes, "origin:firebase/update");
    this.hasRemoteContent = true;
    this.lastPersistedSV = mergeStateVectors(
      this.lastPersistedSV,
  ...
  private writeFirstSnapshot = async (localUpdate: Uint8Array) => {
    ...
    if (result.outcome === "exists") {
      ...
      if (this.snapshotStore && result.contentStoragePath) {
        try {
          remoteBytes = await this.snapshotStore.read({
            path: result.contentStoragePath,
          });
          remoteSv = remoteSv ?? stateVectorFromUpdate(remoteBytes);
        } catch (error) {

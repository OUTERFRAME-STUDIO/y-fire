...
2. Utilizes Firestore as persistent storage and syncs with Firestore periodically to maintain persistent data state. Each shard document stores a compacted Yjs **snapshot** in `content` plus an append-only `updates/*` subcollection of small deltas. A flush writes one small update document (do not flush per keystroke — the trailing `maxWaitFirestoreTime` debounce still applies). Continuous local updates **cannot postpone a flush past `maxFirestoreDeferral`** (default 10s): re-entering `sendToFirestoreQueue` resets the trailing debounce only while elapsed time since the first queued edit is within the cap; past the cap the provider flushes immediately (one coalesced write, not one write per keystroke). Overlapping `saveToFirestore` calls are serialized (`saveInFlight` / `saveQueued`). `appendUpdate` and first-snapshot writes are bounded by `saveTimeoutMs` (default 15s): a hung write reports `save-timeout`, clears `saveInFlight` in `finally`, and retries without advancing `lastPersistedSV`. After each successful append, `lastPersistedSV` is advanced from the document state vector captured at encode time (and remote updates merge an exact per-client `max(clock + length)` from `Y.decodeUpdate` structs, ignoring `Skip`). Do **not** use `Y.encodeStateVectorFromUpdate` for deltas: it drops any client whose structs do not start at clock 0, which would re-encode the whole post-snapshot tail on every flush. A **fold** rewrites `content` to the union of the snapshot, the update docs from the live listener cache, and local state, then deletes only those cached ids. Fold runs **after** a successful append (`onSaving(false)` / `saveInFlight` already cleared) and is serialized tab-wide (one fold at a time) so a production-tab shard pileup cannot hang a save. Fold does **not** bump `contentGeneration` (the epoch): a fold snapshot is a strict superset, so peers apply it as a normal Yjs update. The epoch is reserved for compact/replace, where the new bytes are not a superset. `{ merge: true }` only merges Firestore *fields* — it does not merge CRDT state by itself. Applying `updates/*` in any order converges (`Y.applyUpdate` buffers missing deps); `seq` is for fold windows and debugging only. Optionally pass `snapshotStore` to keep snapshot bytes out of Firestore (see Configuration); the `content` Bytes path remains the default when it is omitted.
...
{documentPath}                         // e.g. projects/{pid}/shards/{docId}
  content            Bytes             // compacted snapshot (default path)
  contentGeneration  number            // epoch; bumped ONLY on replacement
  snapshotSV         Bytes             // state vector of the snapshot
  updatedAt          timestamp
  snapshotBackend    string?           // "storage" when snapshotStore is set
  contentStoragePath string?           // store path; no `content` field then
...
{documentPath}/updates/{autoId}
  ...
  epoch              number?           // writer epoch; omitted on legacy docs
  createdAt          timestamp
{documentPath}/instances/*             // WebRTC signaling, unchanged
```

Hydrate = apply `content` (or `snapshotStore.read` when `snapshotStore` is set), then every `updates/*` doc whose `epoch` is missing or equal to the shard epoch (`contentGeneration`, absent = 0). A numeric `epoch` that differs is ignored (not applied, not counted, not folded, not deleted here). Legacy docs with no `epoch` still apply. Writes wait until a non-cache shard snapshot pins `hydratedEpoch` (a confirmed missing doc is epoch 0; a from-cache miss does not). Snapshot and fold transactions throw `EpochMismatchError` when the shard epoch is not the one this replica hydrated. A permission-denied on append replaces the epoch only after a server read shows contentGeneration advanced; otherwise it is a save failure and the edit is retried. WebRTC Yjs payloads carry that epoch once hydrated; a receiver drops a numeric epoch that differs and still applies a payload with no epoch. The first write on a shard with no snapshot writes the snapshot directly so older readers keep working. After that, each debounced flush appends one update document. `already-exists` on an `updates/*` Create is treated as a committed append (lost-ack / SDK retry), advancing `lastPersistedSV` without `onSaveError`. Fold **decision** uses the live `updates` listener (doc count, total bytes, and cached `{id, update}` bytes) — a normal append and a normal fold do not `getDocs` the collection. When count ≥ `foldUpdateThreshold` (default 20) or total update bytes ≥ `foldBytesFraction` of the 1 MiB cap (default 0.5), the client enqueues a fold **after** `onSaving(false)`: a tab-wide mutex runs one fold at a time; the transaction writes `content = union(server content, local doc, cached update bytes)` (or store metadata when `snapshotStore` is set) and deletes **only** the cached ids. Concurrent appends that land during the fold are left in place and replay harmlessly. Empty deltas are skipped. A delta above 1 MiB does **not** abort: the client force-folds a union snapshot instead (including when `updates/` is empty; `getDocs` only if the listener cache is empty). If that union also exceeds 1 MiB **and** snapshots are stored in Firestore `content`, the provider reports `compact-required` once, backs off further fold attempts (appends keep flowing), and the shard needs DEV-66 compaction. With `snapshotStore`, snapshot bytes skip the 1 MiB Firestore `content` abort (WAL deltas still must be under 1 MiB, else the client force-folds first). `size-warn` (≥ 70% of the cap) applies to Firestore snapshot and fold writes only, not to a large-but-under-cap delta.
...
- **maxWaitFirestoreTime**: Time in milliseconds before triggering persistent data sync to Firestore, defaults to 3000 (trailing debounce; each local re-entry resets this timer)
- **maxFirestoreDeferral**: Maximum milliseconds a Firestore flush can be postponed by trailing-debounce re-entry, defaults to 10000. Past this cap the provider flushes immediately even if edits are still arriving.
- **saveTimeoutMs**: Maximum milliseconds to wait for `appendUpdate` / first-snapshot Firestore writes, defaults to 15000. On timeout the provider reports `onSaveError` with `reason: "save-timeout"` and retries; `lastPersistedSV` is not advanced. The underlying `addDoc` cannot be cancelled — a late success is ignored for the state vector (Yjs CRDT: the retry encodes a superset delta; duplicate `updates/*` rows are merge-safe).
...
- **snapshotStore**: Optional `{ read(meta), write(bytes) }` blob store. When set, first snapshot / fold / replace writes bytes via `snapshotStore.write`, then Firestore metadata only (`snapshotBackend: "storage"`, `contentStoragePath`, optional generation/byte counts, `snapshotSV`, `updatedAt`) — **no** `content` Bytes field. Hydrate uses `snapshotStore.read`. WAL `updates/*`, IndexedDB, WebRTC, `contentGeneration`, and `onEpochReplace` are unchanged. Snapshot bytes skip the 1 MiB Firestore abort; deltas still must be under 1 MiB. Omit this option to keep the default Firestore `content` path.
...
- **onSaveError**: Triggered when an append/snapshot write fails (`save-failed`), a first Firestore `content` snapshot exceeds 1 MiB (`size-abort`), or a fold/forced-snapshot union exceeds 1 MiB (`compact-required`, once per episode). `compact-required` does not block later appends that still fit; the shard needs compaction. With `snapshotStore`, large snapshots do not size-abort. The UI should treat this as unsaved; y-fire retries failed network writes and never falls back to last-write-wins `setDoc`.
